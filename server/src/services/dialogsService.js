...
class DialogsService {
    ...
    try {
      ...
      let data = chatsId.map(async (c) => {
        ...
        let username = messageData
          ? await this.getUsername(messageData.user_id)
          : null;
        return {
          ...chat,
          message: message,
  ...
  async getChatMessages(chatId) {
    try {
      let messagesData = await pool.query(
  ...
  async getChatInfo(chatId) {
    try {
      let chatData = await pool.query("SELECT * FROM chats WHERE id = $1", [
        chatId,
      ]);
